/**
 * Encoding for the external-task reference sesh-web stores in `todoist_task_id`.
 *
 * Mirrors `sesh-web/src/lib/task-ref.ts` — the two have to agree exactly, or a
 * session started from Raycast logs its minutes against the wrong app.
 *
 * That column predates multi-provider support and is used by both `timer_state`
 * and `sessions`. Non-Todoist providers namespace their id (`things:UUID`); a
 * bare id is Todoist, so every existing row keeps working untouched. A session
 * can point at several tasks, so the column carries a comma-separated list —
 * and a list of one encodes to exactly the bare ref it always was.
 */
